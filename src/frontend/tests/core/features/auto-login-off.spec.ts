...
test(
  ...
  async ({ page }) => {
    ...
    await page.waitForSelector("text=sign in to langinfra", { timeout: 30000 });

    await page.getByPlaceholder("Username").fill("langinfra");
    await page.getByPlaceholder("Password").fill("langinfra");
