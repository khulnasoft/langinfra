...
from langinfra.services.chat.config import ChatConfig
...
def set_langchain_cache(settings) -> None:
    ...
    from langinfra.interface.importing.utils import import_class

    if cache_type := os.getenv("LANGINFRA_LANGCHAIN_CACHE"):
        try:
