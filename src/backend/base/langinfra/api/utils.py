...
from langinfra.services.auth.utils import get_current_active_user
from langinfra.services.database.models import User
...
from langinfra.services.database.models.vertex_builds.model import VertexBuildTable
from langinfra.services.deps import async_session_scope, get_async_session, get_session
from langinfra.services.store.utils import get_lf_version_from_pypi

if TYPE_CHECKING:
    from langinfra.services.chat.service import ChatService
    from langinfra.services.store.schema import StoreComponentCreate
...
async def check_langinfra_version(component: StoreComponentCreate) -> None:
    ...
    langinfra_version = await get_lf_version_from_pypi()
    if langinfra_version is None:
        raise HTTPException(status_code=500, detail="Unable to verify the latest version of Langinfra")
    if langinfra_version != component.last_tested_version:
        logger.warning(
            f"Your version of Langinfra ({component.last_tested_version}) is outdated. "
            f"Please update to the latest version ({langinfra_version}) and try again."
        )
