...
def run_on_mac_or_linux(host, port, log_level, options, app):
    webapp_process = Process(target=run_langinfra, args=(host, port, log_level, options, app))
...
def run_on_windows(host, port, log_level, options, app) -> None:
    """Run the Langinfra server on Windows."""
    print_banner(host, port)
    run_langinfra(host, port, log_level, options, app)
...
def build_version_notice(current_version: str, package_name: str) -> str:
    latest_version = fetch_latest_version(package_name, include_prerelease=langinfra_is_pre_release(current_version))
    if latest_version and pkg_version.parse(current_version) < pkg_version.parse(latest_version):
        release_type = "pre-release" if langinfra_is_pre_release(latest_version) else "version"
...
def print_banner(host: str, port: int) -> None:
    ...
    # Use langinfra.utils.version to get the version info
    version_info = get_version_info()
    langinfra_version = version_info["version"]
    package_name = version_info["package"]
    is_pre_release |= langinfra_is_pre_release(langinfra_version)  # Update pre-release status

    notice = build_version_notice(langinfra_version, package_name)
...
def run_langinfra(host, port, log_level, options, app) -> None:
    """Run Langinfra server on localhost."""
    if platform.system() == "Windows":
        import uvicorn
        ...
        from langinfra.server import LanginfraApplication

        server = LanginfraApplication(app, options)
...
def superuser(
    ...
    log_level: str = typer.Option("error", help="Logging level.", envvar="LANGINFRA_LOG_LEVEL"),
) -> None:
    """Create a superuser."""
...
# command to copy the langinfra database from the cache to the current directory
# because now the database is stored per installation
@app.command()
def copy_db() -> None:
    """Copy the database files to the current directory.

    This function copies the 'langinfra.db' and 'langinfra-pre.db' files from the cache directory to the current
    directory.
    ...
    cache_dir = Path(user_cache_dir("langinfra"))
    db_path = cache_dir / "langinfra.db"
    pre_db_path = cache_dir / "langinfra-pre.db"
    ...
    async def aapi_key():
        ...
        async with async_session_scope() as session:
            ...
            if not superuser:
                ...
                return None
            from langinfra.services.database.models.api_key import ApiKey, ApiKeyCreate
            from langinfra.services.database.models.api_key.crud import create_api_key, delete_api_key
...
def show_version(*, value: bool):
    if value:
        ...
        typer.echo(f"langinfra {version}")
        raise typer.Exit
