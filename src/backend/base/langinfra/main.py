...
from langinfra.api import health_check_router, log_router, router
from langinfra.initial_setup.setup import (
...
from langinfra.logging.logger import configure
from langinfra.middleware import ContentSizeLimitMiddleware
from langinfra.services.deps import get_settings_service, get_telemetry_service
from langinfra.services.utils import initialize_services, teardown_services
...
def get_lifespan(*, fix_migration=False, version=None):
    ...
    async def lifespan(_app: FastAPI):
        ...
        # Startup message
        if version:
            rprint(f"[bold green]Starting Langinfra v{version}...[/bold green]")
        else:
            rprint("[bold green]Starting Langinfra...[/bold green]")
        try:
            ...
            if "langinfra migration --fix" not in str(exc):
            ...
            # Final message
            rprint("[bold red]Langinfra shutdown complete[/bold red]")
...
def create_app():
    ...
    app = FastAPI(lifespan=lifespan, title="Langinfra", version=__version__)
    app.add_middleware(
    ...
    if prome_port_str := os.environ.get("LANGINFRA_PROMETHEUS_PORT"):
...
def get_static_files_dir():
    """Get the static files directory relative to Langinfra's main.py file."""
...
if __name__ == "__main__":
    ...
    from langinfra.__main__ import get_number_of_workers

    configure()
    uvicorn.run(
        "langinfra.main:create_app",
        host="127.0.0.1",
        port=7860,
