...
from langinfra.field_typing.constants import CUSTOM_COMPONENT_SUPPORTED_TYPES
...
def create_class(code, class_name):
    ...
    # Replace from langinfra import CustomComponent with from langinfra.custom import CustomComponent
    code = code.replace("from langinfra import CustomComponent", "from langinfra.custom import CustomComponent")
    code = code.replace(
...
def get_default_imports(code_string):
    ...
    langinfra_imports = list(CUSTOM_COMPONENT_SUPPORTED_TYPES.keys())
    necessary_imports = find_names_in_code(code_string, langinfra_imports)
    langinfra_module = importlib.import_module("langinfra.field_typing")
    default_imports.update({name: getattr(langinfra_module, name) for name in necessary_imports})
