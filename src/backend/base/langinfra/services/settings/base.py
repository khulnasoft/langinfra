...
from langinfra.services.settings.constants import VARIABLES_TO_GET_FROM_ENVIRONMENT
from langinfra.utils.util_strings import is_valid_database_url
...
class Settings(BaseSettings):
    # Define the default LANGINFRA_DIR
    config_dir: str | None = None
    # Define if langinfra db should be saved in config dir or
    # in the langinfra directory
    save_db_in_config_dir: bool = False
    """Define if langinfra database should be saved in LANGINFRA_CONFIG_DIR or in the langinfra directory
    (i.e. in the package directory)."""

    dev: bool = False
    """If True, Langinfra will run in development mode."""
    database_url: str | None = None
    """Database URL for Langinfra. If not provided, Langinfra will use a SQLite database."""
    pool_size: int = 10
    ...
    """If set to True, Langinfra will expose Prometheus metrics."""
    prometheus_port: int = 9090
    """The port on which Langinfra will expose Prometheus metrics. 9090 is the default port."""
    ...
    store_url: str | None = "https://api.langinfra.store"
    download_webhook_url: str | None = "https://api.langinfra.store/flows/trigger/ec611a61-8460-4438-b187-a4f65e5559d4"
    like_webhook_url: str | None = "https://api.langinfra.store/flows/trigger/64275852-ec00-45c1-984e-3bff814732da"
    ...
    with the same name in case Langinfra fails to retrieve the variable value."""
    ...
    user_agent: str = "langinfra"
    """User agent for the API calls."""
    backend_only: bool = False
    """If set to True, Langinfra will not serve the frontend."""

    # Telemetry
    do_not_track: bool = False
    """If set to True, Langinfra will not track telemetry."""
    telemetry_base_url: str = "https://langinfra.gateway.scarf.sh"
    transactions_storage_enabled: bool = True
    """If set to True, Langinfra will track transactions between flows."""
    vertex_builds_storage_enabled: bool = True
    """If set to True, Langinfra will keep track of each vertex builds (outputs) in the UI for any flow."""

    # Config
    host: str = "127.0.0.1"
    """The host on which Langinfra will run."""
    port: int = 7860
    """The port on which Langinfra will run."""
    workers: int = 1
    """The number of workers to run."""
    log_level: str = "critical"
    """The log level for Langinfra."""
    log_file: str | None = "logs/langinfra.log"
    """The path to log file for Langinfra."""
    ...
    """If set to True, Langinfra will open the browser on startup."""
    auto_saving: bool = True
    """If set to True, Langinfra will auto save flows."""
    auto_saving_interval: int = 1000
    """The interval in ms at which Langinfra will auto save flows."""
    ...
    """If set to False, Langinfra will not enable the MCP server."""
    mcp_server_enable_progress_notifications: bool = False
    """If set to False, Langinfra will not send progress notifications in the MCP server."""

    @field_validator("dev")
    @classmethod
    def set_dev(cls, value):
        from langinfra.settings import set_dev
    ...
    @classmethod
    def set_langinfra_dir(cls, value):
        if not value:
            from platformdirs import user_cache_dir

            # Define the app name and author
            app_name = "langinfra"
            app_author = "langinfra"

            # Get the cache directory for the application
            cache_dir = user_cache_dir(app_name, app_author)

            # Create a .langinfra directory inside the cache directory
    ...
    def set_database_url(cls, value, info):
        ...
        logger.debug("No database_url provided, trying LANGINFRA_DATABASE_URL env variable")
        if langinfra_database_url := os.getenv("LANGINFRA_DATABASE_URL"):
            value = langinfra_database_url
            logger.debug("Using LANGINFRA_DATABASE_URL env variable.")
        else:
            logger.debug("No database_url env variable, using sqlite database")
            # Originally, we used sqlite:///./langinfra.db
            ...
            from langinfra.utils.version import get_version_info
            from langinfra.utils.version import is_pre_release as langinfra_is_pre_release

            version = get_version_info()["version"]
            is_pre_release = langinfra_is_pre_release(version)
            ...
            if info.data["save_db_in_config_dir"]:
                ...
                logger.debug(f"Saving database to langinfra directory: {database_dir}")

            pre_db_file_name = "langinfra-pre.db"
            db_file_name = "langinfra.db"
    ...
    def set_components_path(cls, value):
        if os.getenv("LANGINFRA_COMPONENTS_PATH"):
            logger.debug("Adding LANGINFRA_COMPONENTS_PATH to components_path")
            langinfra_component_path = os.getenv("LANGINFRA_COMPONENTS_PATH")
            if Path(langinfra_component_path).exists() and langinfra_component_path not in value:
                if isinstance(langinfra_component_path, list):
                    for path in langinfra_component_path:
                        if path not in value:
                            value.append(path)
                    logger.debug(f"Extending {langinfra_component_path} to components_path")
                elif langinfra_component_path not in value:
                    value.append(langinfra_component_path)
                    logger.debug(f"Appending {langinfra_component_path} to components_path")
    ...
    model_config = SettingsConfigDict(validate_assignment=True, extra="ignore", env_prefix="LANGINFRA_")
