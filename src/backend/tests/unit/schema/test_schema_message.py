...
@pytest.fixture
def langinfra_cache_dir(tmp_path):
    """Create a temporary langinfra cache directory."""
    cache_dir = tmp_path / "langinfra"
    cache_dir.mkdir(parents=True)
    return cache_dir


@pytest.fixture
def sample_image(langinfra_cache_dir):
    """Create a sample image file for testing."""
    # Create the test_flow directory in the cache
    flow_dir = langinfra_cache_dir / "test_flow"
    ...
    real_cache_dir = Path(user_cache_dir("langinfra"))
...
def test_message_prompt_serialization():
    template = "Hello, {name}!"
    message = Message.from_template(template, name="Langinfra")
    assert message.text == "Hello, Langinfra!"

    prompt = message.load_lc_prompt()
    assert isinstance(prompt, ChatPromptTemplate)
    assert prompt.messages[0].content == "Hello, Langinfra!"
...
def test_message_with_multiple_images(sample_image, langinfra_cache_dir):
    ...
    real_cache_dir = Path(user_cache_dir("langinfra")) / "test_flow"
    real_cache_dir.mkdir(parents=True, exist_ok=True)
