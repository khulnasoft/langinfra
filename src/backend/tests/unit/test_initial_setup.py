...
import anyio
import pytest
from langinfra.custom.directory_reader.utils import abuild_custom_component_list_from_path
...
from langinfra.interface.types import aget_all_types_dict
...
async def test_refresh_starter_projects():
    data_path = str(await anyio.Path(__file__).parent.parent.parent.absolute() / "base" / "langinfra" / "components")
