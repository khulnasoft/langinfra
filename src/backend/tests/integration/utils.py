...
from langinfra.processing.process import run_graph_internal
...
def download_flow_from_github(name: str, version: str) -> JSONFlow:
    response = requests.get(
        f"https://raw.githubusercontent.com/khulnasoft/langinfra/v{version}/src/backend/base/langinfra/initial_setup/starter_projects/{name}.json",
        timeout=10,
    )
...
def download_component_from_github(module: str, file_name: str, version: str) -> Component:
    ...
    response = requests.get(
        f"https://raw.githubusercontent.com/khulnasoft/langinfra/{version_string}/src/backend/base/langinfra/components/{module}/{file_name}.py",
        timeout=10,
    )
    response.raise_for_status()
