...
from langinfra.services.database.models.folder.model import Folder
...
def blockbuster(request):
    if "benchmark" in request.keywords:
        ...
        with blockbuster_ctx() as bb:
            ...
            bb.functions["os.stat"].can_block_in("langinfra/custom/custom_component/component.py", "set_class_code")
...
def _setup_env(monkeypatch):
    monkeypatch.setenv("LANGINFRA_CACHE_TYPE", "redis")
    monkeypatch.setenv("LANGINFRA_REDIS_HOST", "result_backend")
    monkeypatch.setenv("LANGINFRA_REDIS_PORT", "6379")
    monkeypatch.setenv("LANGINFRA_REDIS_DB", "0")
    monkeypatch.setenv("LANGINFRA_REDIS_EXPIRE", "3600")
    monkeypatch.setenv("LANGINFRA_REDIS_PASSWORD", "")
...
def distributed_client_fixture(
    ...
    from langinfra.core import celery_app

    db_dir = tempfile.mkdtemp()
    try:
        db_path = Path(db_dir) / "test.db"
        monkeypatch.setenv("LANGINFRA_DATABASE_URL", f"sqlite:///{db_path}")
        monkeypatch.setenv("LANGINFRA_AUTO_LOGIN", "false")
        # monkeypatch langinfra.services.task.manager.USE_CELERY to True
        # monkeypatch.setattr(manager, "USE_CELERY", True)
        monkeypatch.setattr(celery_app, "celery_app", celery_app.make_celery("langinfra", Config))
...
def deactivate_tracing(monkeypatch):
    monkeypatch.setenv("LANGINFRA_DEACTIVATE_TRACING", "true")
    yield
    monkeypatch.undo()
